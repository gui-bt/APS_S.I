...
public class SimuladorSO {
    ...
    public static void main(String[] args) {
        ...
        // NOVO: Escolha do escalonador
        System.out.println("\nEscolha o escalonador:");
        System.out.println("1 - FIFO");
        System.out.println("2 - Round-Robin");
        System.out.print("Opção (1 ou 2): ");
        int schedChoice = readInt(sc, 1, 2);
        
        Scheduler scheduler;
        if (schedChoice == 2) {
            System.out.print("Quantum do Round-Robin (ticks, padrão 5): ");
            String qstr = sc.nextLine().trim();
            int quantum = 5;
            if (!qstr.isEmpty()) {
                try { quantum = Math.max(1, Integer.parseInt(qstr)); } catch (Exception ignored) {}
            }
        ...
        while (terminated.size() < all.size()) {
            ...
            // Seleciona processo se não há nenhum rodando
            ...
            if (running != null) {
                ...
                // Verifica se processo terminou CPU
                ...
                // Verifica se precisa fazer I/O
                else if (running.shouldRequestIo()) {
                    moveToWait = true;
                }
                // NOVO: Verifica preempção por quantum (Round-Robin)
                else if (scheduler instanceof RoundRobinScheduler) {
                    RoundRobinScheduler rr = (RoundRobinScheduler) scheduler;
                    if (rr.shouldPreempt() && scheduler.readySize() > 0) {
                        shouldPreempt = true;
                    }
                }

                // Move para waiting se solicitou I/O
                if (moveToWait) {
                ...
                // NOVO: Preempta se quantum expirou
